...
dsBinding x = error $ "malformed binding " ++ printValue x

--allow mapping over scheme lists
scmMap :: (Value -> a) -> Value -> [a]
scmMap f (l :. r) = f l : scmMap f r
scmMap _ Nil = []
scmMap _ x = error $ printValue x ++ " is not a list"

dsBind (Atom v :. expr :. Nil) = Binding v $ dsExpr expr
dsBind bad = error $ "malformed binding " ++ printValue bad
...
dsExpr all@(Atom "lambda" :. args :. body :. Nil) = dsLam args body
    where dsLam Nil body = dsExpr body
          ...
          dsLam _ _ = error $ "malformed lambda " ++ printValue all

dsExpr bad@(Atom "lambda" :. _) = error $ "malformed lambda " ++ printValue bad

dsExpr (Atom "if" :. test :. body1 :. body2 :. Nil) =
    ECase [Alt (dsExpr test) (dsExpr body1), Alt lispTrue (dsExpr body2)]

dsExpr bad@(Atom "if" :. _) = error $ "malformed if " ++ printValue bad

dsExpr (Atom "cond" :. body) = ECase $ scmMap dsAlt body
    where dsAlt (c :. b :. Nil) = Alt (dsExpr c) (dsExpr b)
          dsAlt bad = error $ "malformed condition " ++ printValue bad

dsExpr (Atom "let" :. bindings :. expr :. Nil) = ELet (scmMap dsBind bindings) (dsExpr expr)
dsExpr (Atom "let" :. bad) = error $ "malformed let " ++ printValue bad
...
dsExpr (Atom "letrec" :. bad) = error $ "malformed letrec " ++ printValue bad
...
dsExpr x = error $ "internal error - unexpected form \n" ++ printValue x
